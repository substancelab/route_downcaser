...
== Optionally redirect requests to downcased URI

To configure the gem so that the request is redirected to the downcased path, for SEO purposes for example, set the redirect flag in an initializer (Rails) or in application.rb (Sinatra).
